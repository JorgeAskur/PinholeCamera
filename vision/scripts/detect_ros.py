...
class Yolov7Publisher:
    def __init__(self, img_topic: str, weights: str, conf_thresh: float = 0.5,
        ...
        self.img_depth_subscriber = rospy.Subscriber("/zed2i/zed_node/depth/depth_registered", Image, self.depthCB)
        self.detection_publisher = rospy.Publisher(
            "pub_topic", Detection2DArray, queue_size=queue_size
        )

        self.objects_publisher = rospy.Publisher("objects_detected", obj_detected_list, queue_size=queue_size)
        rospy.loginfo("YOLOv7 initialization complete. Ready to start inference")
    
    def depthCB(self, img):
        self.depthImg = self.bridge.imgmsg_to_cv2(img, img.encoding)
    ...
    def process_img_msg(self, img_msg: Image):
        ...
        # blue_img  = np.full((360,640,3), (255,0,0), np.uint8)
        # np_img_orig  = cv2.addWeighted(np_img_orig, 0.8, blue_img, 0.2, 0)
        ...
        for element in detection_msg.detections:
            ...
            if obj.clase == 'person':
                ...
                self.objects_id = self.objects_id
